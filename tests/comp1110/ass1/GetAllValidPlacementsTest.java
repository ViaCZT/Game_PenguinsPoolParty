...
public class GetAllValidPlacementsTest {
    ...
    private void test(PenguinsPoolParty[] boards, String[][] expected) {
        for (int i = 0; i < boards.length; i++) {
            PenguinsPoolParty penguinsPoolParty = boards[i];
            String[] actual = penguinsPoolParty.getAllValidPlacements();
            assertArrayEquals(expected[i], actual, "For board state " + boards[i].boardToString() +
                    ", expected valid placements " + Arrays.toString(expected[i]) +
                    " but actually got " + Arrays.toString(actual) + "!");
        }
    }

    @Test
    public void testStarter() {
        test(STARTER_BOARDS, STARTER_EXPECTED_PLACEMENTS);
    }

    @Test
    public void testJunior() {
        test(JUNIOR_BOARDS, JUNIOR_EXPECTED_PLACEMENTS);
    }

    @Test
    public void testExpert() {
        test(EXPERT_BOARDS, EXPERT_EXPECTED_PLACEMENTS);
    }

    @Test
    public void testMaster() {
        test(MASTER_BOARDS, MASTER_EXPECTED_PLACEMENTS);
    }
